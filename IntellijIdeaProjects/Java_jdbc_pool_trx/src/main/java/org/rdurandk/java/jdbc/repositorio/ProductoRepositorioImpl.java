...
public class ProductoRepositorioImpl implements Repositorio<Producto> {
    ...
    public List<Producto> listar() throws SQLException {
        ...
        try(Statement stmt = conn.createStatement();//obtengo la conexion por statement
    ...
    public Producto porId(Long id) throws SQLException {
        Producto p = null;
        try(PreparedStatement stmt = conn.prepareStatement("SELECT p.*, c.nombre as nom_categoria FROM productos as p "+
    ...
    public Producto guardar(Producto producto) throws SQLException {
        ...
        try(PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)){
            ...
            if(producto.getId() == null){
                try(ResultSet rs = ps.getGeneratedKeys()){//obtener el key o la llave primaria
                    if(rs.next()){
                        producto.setId(rs.getLong(1));//en este caso 1
                    }
                }
            }
            return producto;
